      ...
      >
        {[...coffeeTypes, ...coffeeTypes].map((type, index) => (
          <div key={index} className="flex items-center space-x-2 md:space-x-3 whitespace-nowrap">
            <div className="w-6 h-6 md:w-8 md:h-8 bg-amber-100 rounded-full flex items-center justify-center">
              <span className="text-amber-800 text-sm md:text-lg">☕</span>
            </div>
            <span className="font-bold tracking-wider text-lg md:text-2xl text-white">
              {type}
            </span>
          </div>
        ))}
